...
function sline_breadcrumb($breadcrumb) {
	...
	if (is_array ( $breadcrumb ['breadcrumb'] )) {
		...
		foreach ( $breadcrumb ['breadcrumb'] as $bitem ) {
			$output .= '<li>' . $bitem . '</li>';
		}
		$output .= '<li><span>' . $title . '</span></li>';
		$output .= '</ul>';
	}
	return $output;
}

function sline_file_link($variables) {
  $file = $variables['file'];
  $url = file_create_url($file->uri);
  $classList = explode("/", $file->filemime);
  ...
  $link_text .= '<br />' . format_size($file->filesize);
