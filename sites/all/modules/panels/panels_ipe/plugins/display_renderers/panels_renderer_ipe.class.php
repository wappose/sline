...
class panels_renderer_ipe extends panels_renderer_editor {
	...
	function ajax_save_form($break = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'content_types' => $this->cache->content_types, 'rerender' => FALSE, 'no_redirect' => TRUE, // Panels needs this to make sure that the layout gets callbacks
...
function panels_ipe_edit_control_form($form, &$form_state) {
	...
	foreach ( $layout_panels as $panel_id => $title ) {
		...
		$form ['panel'] ['pane'] [$panel_id] = array (// Use 'hidden' instead of 'value' so the js can access it.
		'#type' => 'hidden', '#default_value' => implode ( ',', ( array ) $display->panels [$panel_id] ) );
	}
	...
	$form ['buttons'] ['cancel'] = array ('#type' => 'submit', '#id' => 'panels-ipe-cancel', '#value' => t ( 'Cancel' ) );
	return $form;
}
