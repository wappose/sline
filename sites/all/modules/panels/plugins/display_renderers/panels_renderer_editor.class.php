...
class panels_renderer_editor extends panels_renderer_standard {
	...
	function edit() {
		$form_state = array ('display' => &$this->display, 'renderer' => &$this, 'content_types' => $this->cache->content_types, 'no_redirect' => TRUE, 'display_title' => ! empty ( $this->cache->display_title ), 'cache key' => $this->display->cache_key );
	...
	function get_style_links($type, $id = NULL) {
		...
		$style_links [] = array ('title' => $style_title, 'attributes' => array ('class' => array ('panels-text' ) ) );
		
		$style_links [] = array ('title' => t ( 'Change' ), 'href' => $this->get_url ( 'style-type', $type, $id ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
		
		$function = $type != 'pane' ? 'settings form' : 'pane settings form';
		if (panels_plugin_get_function ( 'styles', $style, $function )) {
			$style_links [] = array ('title' => t ( 'Settings' ), 'href' => $this->get_url ( 'style-settings', $type, $id ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
		}
	...
	function get_display_links() {
		...
		if (user_access ( 'administer panels styles' )) {
			...
			$links [] = array ('title' => '<span class="dropdown-header">' . t ( 'Style' ) . '</span>' . theme_links ( array ('links' => $style_links, 'attributes' => array (), 'heading' => array () ) ), 'html' => TRUE, 'attributes' => array ('class' => array ('panels-sub-menu' ) ) );
		}
		
		if (user_access ( 'use panels caching features' )) {
			$links [] = array ('title' => '<hr />', 'html' => TRUE );
			...
			$cache_links [] = array ('title' => $cache_method, 'attributes' => array ('class' => array ('panels-text' ) ) );
			$cache_links [] = array ('title' => t ( 'Change' ), 'href' => $this->get_url ( 'cache-method', 'display' ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			if (panels_plugin_get_function ( 'cache', $info, 'settings form' )) {
				$cache_links [] = array ('title' => t ( 'Settings' ), 'href' => $this->get_url ( 'cache-settings', 'display' ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			}
			
			$links [] = array ('title' => '<span class="dropdown-header">' . t ( 'Caching' ) . '</span>' . theme_links ( array ('links' => $cache_links, 'attributes' => array (), 'heading' => array () ) ), 'html' => TRUE, 'attributes' => array ('class' => array ('panels-sub-menu' ) ) );
		}
	...
	function get_region_links($region_id) {
		$links = array ();
		$links [] = array ('title' => t ( 'Add content' ), 'href' => $this->get_url ( 'select-content', $region_id ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
	...
	function get_pane_links($pane, $content_type) {
		...
		if (! empty ( $pane->shown )) {
			$links [] = array ('title' => t ( 'Disable this pane' ), 'href' => $this->get_url ( 'hide', $pane->pid ), 'attributes' => array ('class' => array ('use-ajax' ) ) );
		} else {
			$links [] = array ('title' => t ( 'Enable this pane' ), 'href' => $this->get_url ( 'show', $pane->pid ), 'attributes' => array ('class' => array ('use-ajax' ) ) );
		}
		
		if (isset ( $this->display->title_pane ) && $this->display->title_pane == $pane->pid) {
			$links ['panels-set-title'] = array ('title' => t ( '&#x2713;Panel title' ), 'html' => TRUE );
		} else {
			$links ['panels-set-title'] = array ('title' => t ( 'Panel title' ), 'href' => $this->get_url ( 'panel-title', $pane->pid ), 'attributes' => array ('class' => array ('use-ajax' ) ) );
		}
		
		$subtype = ctools_content_get_subtype ( $content_type, $pane->subtype );
		
		if (ctools_content_editable ( $content_type, $subtype, $pane->configuration )) {
			$links [] = array ('title' => isset ( $content_type ['edit text'] ) ? $content_type ['edit text'] : t ( 'Settings' ), 'href' => $this->get_url ( 'edit-pane', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
		}
		
		if (user_access ( 'administer advanced pane settings' )) {
			$links [] = array ('title' => t ( 'CSS properties' ), 'href' => $this->get_url ( 'pane-css', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
		}
		...
		if (user_access ( 'administer pane access' )) {
			...
			if (empty ( $visibility_links )) {
				$visibility_links [] = array ('title' => t ( 'No rules' ), 'attributes' => array ('class' => array ('panels-text' ) ) );
			}
			
			$visibility_links [] = array ('title' => t ( 'Add new rule' ), 'href' => $this->get_url ( 'access-add-test', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			
			$visibility_links [] = array ('title' => t ( 'Settings' ), 'href' => $this->get_url ( 'access-settings', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			
			$links [] = array ('title' => '<span class="dropdown-header">' . t ( 'Visibility rules' ) . '</span>' . theme_links ( array ('links' => $visibility_links, 'attributes' => array (), 'heading' => array () ) ), 'html' => TRUE, 'attributes' => array ('class' => array ('panels-sub-menu' ) ) );
		}
		...
		if (panels_get_caches () && user_access ( 'use panels caching features' )) {
			...
			$cache_method = isset ( $info ['title'] ) ? $info ['title'] : t ( 'No caching' );
			$cache_links [] = array ('title' => $cache_method, 'attributes' => array ('class' => array ('panels-text' ) ) );
			$cache_links [] = array ('title' => t ( 'Change' ), 'href' => $this->get_url ( 'cache-method', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			if (panels_plugin_get_function ( 'cache', $info, 'settings form' )) {
				$cache_links [] = array ('title' => t ( 'Settings' ), 'href' => $this->get_url ( 'cache-settings', $pane->pid ), 'attributes' => array ('class' => array ('ctools-use-modal' ) ) );
			}
		...
		$links [] = array ('title' => t ( 'Remove' ), 'href' => '#', 'attributes' => array ('class' => array ('pane-delete' ), 'id' => "pane-delete-panel-pane-$pane->pid" ) );
		...
		foreach ( $content_types as $type_name => $subtypes ) {
			foreach ( $subtypes as $subtype_name => $content_type ) {
				...
				if (empty ( $categories [$category_key] )) {
					$categories [$category_key] = array ('title' => $category, 'content' => array () );
	...
	function ajax_add_pane($region = NULL, $type_name = NULL, $subtype_name = NULL, $step = NULL) {
		...
		$form_state = array ('display' => &$this->cache->display, 'contexts' => $this->cache->display->context, 'pane' => &$pane, 'cache_key' => $this->display->cache_key, 'display cache' => &$this->cache, 'ajax' => TRUE, 'modal' => TRUE, // This will force the system to not automatically render.
		'modal return' => TRUE, 'commands' => array () );
	...
	function ajax_edit_pane($pid = NULL, $step = NULL) {
		...
		$form_info = array ('path' => $this->get_url ( 'edit-pane', $pid, '%step' ), 'show cancel' => TRUE, 'next callback' => 'panels_ajax_edit_pane_next', 'finish callback' => 'panels_ajax_edit_pane_finish', 'cancel callback' => 'panels_ajax_edit_pane_cancel' );
	...
	function ajax_cache_method($pid = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'conf' => &$conf, 'title' => $title, 'ajax' => TRUE );
	...
	function ajax_cache_settings($pid = 0) {
		...
		$form_state = array ('display' => &$this->display, 'pid' => $pid, 'conf' => &$conf, 'ajax' => TRUE, 'title' => $title, 'url' => url ( $this->get_url ( 'cache-settings', $pid ), array ('absolute' => TRUE ) ) );
	...
	function ajax_style_type($type, $pid = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'style' => $style, 'title' => $title, 'ajax' => TRUE, 'type' => $type );
	...
	function ajax_style_settings($type, $pid = '') {
		...
		$form_state = array ('display' => &$this->display, 'type' => $type, 'pid' => $pid, 'conf' => &$conf, 'style' => $style, 'ajax' => TRUE, 'title' => $title, 'url' => url ( $this->get_url ( 'style-settings', $type, $pid ), array ('absolute' => TRUE ) ), 'renderer' => &$this );
	...
	function ajax_pane_css($pid = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'pane' => &$pane, 'ajax' => TRUE, 'title' => t ( 'Configure CSS on !subtype_title', array ('!subtype_title' => $subtype ['title'] ) ) );
	...
	function ajax_access_settings($pid = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'pane' => &$pane, 'ajax' => TRUE, 'title' => t ( 'Access settings on !subtype_title', array ('!subtype_title' => $subtype ['title'] ) ) );
	...
	function ajax_access_configure_test($pid = NULL, $id = NULL) {
		...
		$form_state = array ('display' => &$this->display, 'pane' => &$pane, 'ajax' => TRUE, 'title' => t ( 'Configure visibility rule for !subtype_title', array ('!subtype_title' => $subtype ['title'] ) ), 'test' => &$pane->access ['plugins'] [$id], 'plugin' => ctools_get_access_plugin ( $pane->access ['plugins'] [$id] ['name'] ), 'url' => url ( $url, array ('absolute' => TRUE ) ) );
...
function panels_edit_cache_method_form($form, &$form_state) {
	...
	$form ['method'] = array ('#prefix' => '<div class="no-float">', '#suffix' => '</div>', '#type' => 'radios', '#title' => t ( 'Method' ), '#options' => $options, '#default_value' => $conf ['method'] );
...
function panels_edit_cache_settings_form($form, &$form_state) {
	...
	$form ['description'] = array ('#prefix' => '<div class="description">', '#suffix' => '</div>', '#value' => check_plain ( $info ['description'] ) );
...
function panels_edit_style_type_form($form, &$form_state) {
	...
	$form ['style'] = array ('#prefix' => '<div class="no-float">', '#suffix' => '</div>', '#type' => 'radios', '#title' => t ( 'Style' ), '#options' => $options, '#default_value' => $style );
...
function panels_edit_style_settings_form($form, &$form_state) {
	...
	$form ['description'] = array ('#prefix' => '<div class="description">', '#suffix' => '</div>', '#value' => check_plain ( $style ['description'] ) );
...
function panels_edit_configure_pane_css_form($form, &$form_state) {
	...
	$form ['css_id'] = array ('#type' => 'textfield', '#default_value' => isset ( $pane->css ['css_id'] ) ? $pane->css ['css_id'] : '', '#title' => t ( 'CSS ID' ), '#description' => t ( 'CSS ID to apply to this pane. This may be blank.' ) );
	$form ['css_class'] = array ('#type' => 'textfield', '#default_value' => isset ( $pane->css ['css_class'] ) ? $pane->css ['css_class'] : '', '#title' => t ( 'CSS class' ), '#description' => t ( 'CSS class to apply to this pane. This may be blank.' ) );
...
function panels_edit_configure_access_settings_form(&$form_state) {
	...
	$form ['logic'] = array ('#type' => 'radios', '#options' => array ('and' => t ( 'All criteria must pass.' ), 'or' => t ( 'Only one criterion must pass.' ) ), '#default_value' => isset ( $pane->access ['logic'] ) ? $pane->access ['logic'] : 'and' );
...
function panels_edit_add_access_test_form($form, &$form_state) {
	...
	$form ['type'] = array (// This ensures that the form item is added to the URL.
	'#type' => 'radios', '#options' => $options );
